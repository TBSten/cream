 ...
 * @property funName Template for the generated bridge function's name, as for cream's
 *   copy/combine annotations. Defaults to [DefaultCopyFunctionName], which here expands to the
 *   annotated function's own name (the overload behaviour) — so `funName =
 *   DefaultCopyFunctionName + "FromArgs"` derives a name from it. The `CopyTarget*` tokens are
 *   NOT supported, because `@CallFrom` has no target class to render; embedding one is a
 *   positioned compilation error. When two bridges would end up with the same
