 ...
 * this annotation** (so the two never emit conflicting duplicate accessors). Its arguments
 * layer over this annotation's, most specific first: for each of [propertyName] / [kdoc] /
 * [visibility], the property's own value wins when set, otherwise this annotation's, otherwise
 * cream's default. An argument left at its default (the [DefaultAccessorPropertyName] token,
 * [CopyVisibility.INHERIT], an empty [KDoc]) counts as unset — so adding `@ParentOptional`
 * purely to rename one property does not silently drop the sweep's [kdoc] / [visibility].
 *
 * These arguments apply to the accessors **this** annotation generates; a sealed ancestor that
 * is not itself annotated generates its own through [ParentOptional].
 *
 * # Exclude
 *
 ...
 *
 * @property propertyName Template for every generated accessor's name, applied to the whole
 *   swept hierarchy. Defaults to [DefaultAccessorPropertyName] (each accessor keeps its base
 *   property's own name); set `propertyName = "${DefaultAccessorPropertyName}OrNull"` to suffix
 *   every accessor at once. A property's own [ParentOptional.propertyName] overrides this.
