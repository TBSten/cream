...
| サブ | 置くもの |
|---|---|
| `core/common/` | 全生成系で共有する部品: 型パラメータ(header/where 描画)・プロパティマッチング・KDoc 生成・命名（shared へのブリッジ）・identifier escape の呼び出し・target 検証・visibility・診断ヘルパ・`GenerateSourceAnnotation`（+ その 8 実装 `<Name>SourceAnnotation`。sealed ではなく overridable member で注釈ごとの差分を表す）・`FindMappedSourceProperty` / `IsExcluded`（生成側が引数で受け取るパラメータ単位の挙動） |
