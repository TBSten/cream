...
- `GenerateSourceAnnotation`（`core/common/GenerateSourceAnnotation.kt`, package `me.tbsten.cream.ksp.core.common`）: 生成元注釈を識別し、その注釈固有の生成ルールを持つ。実装は `<Name>SourceAnnotation`（`CopySourceAnnotation.kt` / `CombineSourceAnnotation.kt` / `MappingSourceAnnotation.kt` / `CallFromSourceAnnotation.kt` に family 別で 9 個）。
