...
Suite built: `feature/callFrom/scenario/` (19 files / 58 scenarios / 174 goldens). The only
...
- Annotated: the **target function** carries `@CallFrom(vararg sources)` (`@Target(FUNCTION)`); each
  ...
  *parameters* (`x = x`), so caller overrides win. Member functions bridge as top-level **extension
  ...
  Not `@Repeatable` → no repeatable family. `funName` IS supported: the default sentinel resolves to
  the annotated function's own name (the same-name overload); any other value is a **plain literal**
  used verbatim — `CopyTarget*` tokens don't apply because there is no target *class* to render
  (`resolveCallFromFunName` in `core/callFrom`) — renaming the bridge while its body still delegates.
...
- **Families used (18)**: sourceKind, **functionKind** (recast targetKind: top-level / member /
  ...
  **returnShape** (Nothing / nullable), **typealiasShape** (aliased param / property; needs
  ...
  property → `@Deprecated` propagated onto the bridge), **funName** (default same-name overload /
  custom literal name / one custom name across multiple sources — still N independent overloads).
  Dropped: targetKind/constructor (recast), repeatable (annotation lacks it); context parameters
  (invisible to the KSP API — documented limitation, nothing observable to snapshot).
- All 174 goldens exit OK (rejects live in `CallFromInvalidUsageTest` diagnostics: private/protected /
  ...
  cap / duplicate bridge signatures — incl. a custom funName shared by two functions — / clash with
  an existing user-written overload).
...
- Existing test data: `test/src/commonTest/.../callFrom/{Basic,Vararg,Extension,Defaults,FunName,Modifiers,Shapes}Test.kt`
