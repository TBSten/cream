...
// DefaultAccessorPropertyName トークンは元プロパティ名に解決されるため、
// 各プロパティ名を書き下さずに接尾辞だけでアクセサ名を導出できる
sealed interface Fetch {
    data class Loaded(
        @ParentOptional(propertyName = DefaultAccessorPropertyName + "OrNull") val body: String,
        @ParentOptional(propertyName = "${DefaultAccessorPropertyName}OrNull") val status: Int,
    ) : Fetch
