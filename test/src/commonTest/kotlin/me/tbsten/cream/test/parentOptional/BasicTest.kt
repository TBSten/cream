...
class BasicTest :
    FreeSpec({
        ...
        "DefaultAccessorPropertyName トークンで元プロパティ名から導出したアクセサ名が生成される" {
            val loaded: Fetch = Fetch.Loaded(body = "payload", status = 200)
            val pending: Fetch = Fetch.Pending
            // `+` 連結・文字列テンプレートのどちらの書き方でも同じ名前に解決される
            loaded.bodyOrNull shouldBe "payload"
            loaded.statusOrNull shouldBe 200
            pending.bodyOrNull shouldBe null
            pending.statusOrNull shouldBe null
        }
