...
`funName` is a **template string**, as for the copy annotations. `DefaultCopyFunctionName` — the
argument's default — expands here to the annotated function's own name, so omitting `funName` keeps
the overload and embedding it derives a name from it:

```kt
@CallFrom(BuildConfigArgs::class, funName = DefaultCopyFunctionName + "FromArgs")
fun buildConfig(name: String, size: Int): String = "$name:$size"
// generates: fun buildConfigFromArgs(buildConfigArgs: BuildConfigArgs, ...)
```

(This mirrors [`@SealedCopy`](./sealed-copy.md), which expands the same token to `copy`.) The
`CopyTarget*` tokens are **not** supported — there is no target class to render — and embedding one
is a positioned compile error rather than a broken generated name. The module-wide naming options
(`cream.copyFunNamePrefix`, `cream.copyFunNamingStrategy`, `cream.escapeDot`) do not affect
`@CallFrom` either. Bridges from different source classes never collide on a shared `funName` — each has a distinct
...
- **No `CopyTarget*` naming tokens.** [`funName`](#custom-bridge-name-funname) is a template and
  `DefaultCopyFunctionName` expands to the annotated function's own name, but the `CopyTarget*`
  tokens render a target class `@CallFrom` does not have (embedding one is a positioned error), and
