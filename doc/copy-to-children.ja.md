...
- sealed 親に宣言したプロパティに `@CopyToChildren.Map` を付与すると、子クラスの別名の
  コンストラクタパラメータに紐付けられ、名前が違っても **すべての** per-child コピー関数で
  自動コピーのデフォルト値 (`= this.<プロパティ>`) が付きます。詳細は
  [Property mapping](./customization/property-mapping.ja.md) を参照してください。
...
- [Property mapping](./customization/property-mapping.ja.md) — `@CopyToChildren.Map` とその他の `.Map` アノテーション。
