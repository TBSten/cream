...
`funName` は、コピー系の注釈と同じく **テンプレート文字列** です。引数のデフォルト値でもある
`DefaultCopyFunctionName` は、ここでは注釈を付けた関数自身の名前に解決されます。つまり
`funName` を省略すればオーバーロードのまま、埋め込めばそこから名前を導出できます:

```kt
@CallFrom(BuildConfigArgs::class, funName = DefaultCopyFunctionName + "FromArgs")
fun buildConfig(name: String, size: Int): String = "$name:$size"
// 生成: fun buildConfigFromArgs(buildConfigArgs: BuildConfigArgs, ...)
```

（同じトークンを `copy` に展開する [`@SealedCopy`](./sealed-copy.ja.md) と同じ仕組みです。）
`CopyTarget*` トークンには **対応しません** — レンダリングする対象クラスが無いためで、埋め込むと
壊れた名前を生成する代わりに位置付きのコンパイルエラーになります。モジュール全体の命名オプション
（`cream.copyFunNamePrefix`、`cream.copyFunNamingStrategy`、`cream.escapeDot`）も `@CallFrom` には
...
- **`CopyTarget*` 命名トークンには対応しません。** [`funName`](#ブリッジ名のカスタマイズfunname)
  はテンプレートで `DefaultCopyFunctionName` は元関数名に解決されますが、`CopyTarget*` トークンは
  `@CallFrom` に存在しない対象クラスを描画するものなので、埋め込むと位置付きエラーになります。
