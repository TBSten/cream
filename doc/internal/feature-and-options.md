...
| オプション | プロジェクト単位 (`ksp.arg`) | CopyTo | CopyFrom | CopyToChildren | SealedCopy | CombineTo | CombineFrom | CopyMapping | CombineMapping | CallFrom |
|---|---|:--:|:--:|:--:|:--:|:--:|:--:|:--:|:--:|:--:|
| **命名 — 生成関数の名前** | | | | | | | | | | |
...
| canReverse | ✕ 無 | ✕ | ✕ | ✕ | ✕ | ✕ | ✕ | ✓ | ✕ | ✕ |
| **プロパティ単位の制御** | | | | | | | | | | |
| 名前マッピング (`.Map`/`properties`) | ✕ 無 | ✓ Map | ✓ Map | ✓ Map | ✓ Map | ✓ Map | ✓ Map | ✓ prop | ✓ prop | ✓ Map |
| デリゲート選択 (`.Via`) | ✕ 無 | ✕ | ✕ | ✕ | ✓ | ✕ | ✕ | ✕ | ✕ | ✕ |
| 自動コピー除外 (`.Exclude`) | ✕ 無 | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ excludes | ✓ excludes | ✓ |
| **対象の指定** | | | | | | | | | | |
| 対象指定 targets/sources | —（必須引数） | ✓ | ✓ | —(自身) | —(自身) | ✓ | ✓ | ✓ | ✓ | ✓ |

> **CallFrom の注**: `@CallFrom` は関数に付与し、生成物は既定では元関数と同名のオーバーロード。
> `funName` 引数で名前を上書きできる（リテラルのみ。コピー先がクラスでないため `CopyTarget*` トークンは非対応で、
> 既定値も上記3つの合成ではなく元関数名）。ただし命名系のプロジェクトオプション（prefix / strategy / escapeDot）は
> CallFrom には作用しない。コピー先がクラスでないため `notCopyToObject` も作用しない。
