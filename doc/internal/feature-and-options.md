...
> v0.9.0-alpha01 時点の情報 + #135 の @ParentOptional。出典は `cream-runtime` の注釈定義と `CreamOptions`（`cream-ksp/shared`）。

凡例: **✓** = その単位で指定可 ／ **✕** = 該当引数なし ／ **適用** = 引数は無いがプロジェクト設定が効く ／ **✱** = 別オプションが担当

| オプション | プロジェクト単位 (`ksp.arg`) | CopyTo | CopyFrom | CopyToChildren | SealedCopy | CombineTo | CombineFrom | CopyMapping | CombineMapping | ParentOptional |
|---|---|:--:|:--:|:--:|:--:|:--:|:--:|:--:|:--:|:--:|
| **命名 — 生成関数の名前** | | | | | | | | | | |
...
| **対象の指定** | | | | | | | | | | |
| 対象指定 targets/sources | —（必須引数） | ✓ | ✓ | —(自身) | —(自身) | ✓ | ✓ | ✓ | ✓ | —(sealed 祖先) |

✱ (ParentOptional 列): 生成するのは関数ではなく拡張プロパティ。名前の個別上書きは `funName` ではなく `@ParentOptional(propertyName = ...)` が担当。
