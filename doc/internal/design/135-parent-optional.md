...
public annotation class ParentOptional(
    val propertyName: String = DefaultAccessorPropertyName,   // トークン = 元プロパティと同名
    val kdoc: KDoc = KDoc(),
...
- `propertyName` は `funName` と同じ **テンプレート文字列**。`DefaultAccessorPropertyName`（`"{{cream:DefaultAccessorPropertyName}}"`、`AccessorPropertyNameToken.kt`）が注釈対象プロパティ名に置換される。トークン自体がデフォルト値なので「省略 = 元の名前」が引数のデフォルトとして自然に表現される。命名は `funName` 側の `Default<生成物>Name` 規則（`DefaultCopyFunctionName` / `DefaultCallFromFunctionName`）に揃えた。解決は `ParentOptionalSourceAnnotation.accessorName`（`replace` 1 回）で、`funName` と同じく識別子としての妥当性は検証しない。
...
| core/common | `ParentOptionalSourceAnnotation.kt` に `ParentOptionalSourceAnnotation` / `ChildOptionalsSourceAnnotation`（`GenerateSourceAnnotation` 実装）を追加。unsealed interface（#185）なので `when` 分岐追加は不要 — 全 rule が既定値のままで、accessor 生成のため `findMappedSourceProperty` / `isExcluded` も供給しない |
