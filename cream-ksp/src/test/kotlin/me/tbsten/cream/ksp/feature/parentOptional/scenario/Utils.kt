 ...
 *
 * [propertyName] は文字列リテラルとして描画される。[DefaultAccessorPropertyName] トークンを
 * 含むテンプレートを書きたい場合は [propertyNameCode] に式を渡す（両方指定は不可）。
 */
internal fun parentOptional(
    propertyName: String? = null,
    visibility: CopyVisibility? = null,
    kdoc: CodeBlock? = null,
    propertyNameCode: CodeBlock? = null,
): AnnotationSpec =
    AnnotationSpec
        .builder(ParentOptional::class)
        .apply {
            require(propertyName == null || propertyNameCode == null) {
                "Pass either propertyName or propertyNameCode, not both."
            }
            if (propertyName != null) addMember("%L = %S", ParentOptional::propertyName.name, propertyName)
            if (propertyNameCode != null) addMember("%L = %L", ParentOptional::propertyName.name, propertyNameCode)
...
internal fun parentOptionalProp(
    ...
    kdoc: CodeBlock? = null,
    propertyNameCode: CodeBlock? = null,
): Prop = Prop(name, type, paramAnnotation = parentOptional(propertyName, visibility, kdoc, propertyNameCode))

/** `propertyName = DefaultAccessorPropertyName + "<[suffix]>"` を描画する式。 */
internal fun defaultAccessorPropertyNamePlus(suffix: String): CodeBlock =
    CodeBlock.of(
        "%M + %S",
        MemberName("me.tbsten.cream", "DefaultAccessorPropertyName"),
        suffix,
    )
