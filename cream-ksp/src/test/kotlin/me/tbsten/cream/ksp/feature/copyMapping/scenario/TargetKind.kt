...
internal fun targetKindScenarios(): Generator<SnapshotScenario> {
    ...
    return Generator.snapshotScenarios(
        "objectTarget" to copyMapping(mappingHolder(), source, TypeSpec.objectBuilder("Target").build()),
        "sealedInterfaceTarget" to copyMapping(mappingHolder(), source, sealedInterfaceTarget()),
        "abstractTarget" to
            copyMapping(mappingHolder(), source, clazz("Target", Prop("name"), Prop("extra", INT), modifiers = listOf(ABSTRACT))),
        "nonSealedInterfaceTarget" to
            copyMapping(
                mappingHolder(),
                source,
                TypeSpec.interfaceBuilder("Target").addProperty(PropertySpec.builder("name", STRING).build()).build(),
            ),
        "enumTarget" to
            copyMapping(
                mappingHolder(),
        ...
        "privateConstructorTarget" to
            copyMapping(mappingHolder(), source, clazz("Target", Prop("name"), Prop("extra", INT), constructorVisibility = PRIVATE)),
    )
}
