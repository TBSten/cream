...
/** [this] を sealed parent として `@ChildOptionals`（任意で `propertyName` / `visibility` / `kdoc`）を付与する。 */
    ...
    toBuilder()
        .addAnnotation(
            AnnotationSpec
                ...
                .apply {
                    if (propertyNameCode != null) addMember("%L = %L", ChildOptionals::propertyName.name, propertyNameCode)
...
internal fun childOptionals(
    ...
    SnapshotScenario(
        listOf(sealedParent.withChildOptionals(visibility, kdoc, propertyNameCode)) + additionalTopLevel,
    )

/** `DefaultAccessorPropertyName + "<[suffix]>"` を描画する式（sweep 全体のリネームテンプレート）。 */
