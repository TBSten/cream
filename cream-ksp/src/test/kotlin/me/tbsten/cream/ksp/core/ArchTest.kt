 ...
 *   instead. Files live only in `common`/`copyFun`/`combineFun`/`sealedCopy`/`parentOptional`.
...
internal class ArchTest :
    FreeSpec({
        "core レイヤ" - {
            ...
            "common / copyFun / combineFun / sealedCopy / parentOptional サブパッケージにのみ置く（core/ 直下に .kt を置かない）" {
                creamKspMain
                    .filter { it.inLayer(CORE_PACKAGE) }
