 ...
 *   instead. Files live only in `common`/`copyFun`/`combineFun`/`sealedCopy`/`parentOptional`/`error`.
...
internal class ArchTest :
    FreeSpec({
        "core レイヤ" - {
            ...
            "common / copyFun / combineFun / sealedCopy / parentOptional / error サブパッケージにのみ置く（core/ 直下に .kt を置かない）" {
                creamKspMain
                    .filter { it.inLayer(CORE_PACKAGE) }
