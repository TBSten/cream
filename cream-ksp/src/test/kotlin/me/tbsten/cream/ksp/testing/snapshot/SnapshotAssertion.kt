...
/**
 * [TestScope] convenience: derive the snapshot [name] from the running test, so callers inside a
 * `test("...") { }` block need not repeat it. The path is `<SpecName>/<testName>.md` — the spec's
 * simple name becomes the directory (first `.` boundary), the test name the file. An optional
 * [nameSuffix] appends a `/`-separated segment (e.g. a facet variant).
 *
 * Note: use `testCase.name.name` (the `TestName`'s string), not the `TestName` object whose
 * `toString()` is `TestName(name=..., focus=..., ...)` and would leak into the file name.
 */
internal fun TestScope.assertMatchesSnapshot(
    ...
    val specName =
        "${testCase.spec::class.simpleName ?: "Unknown Test Spec"}/" +
