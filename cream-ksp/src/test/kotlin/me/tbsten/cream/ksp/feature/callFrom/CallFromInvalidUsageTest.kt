...
internal class CallFromInvalidUsageTest :
    FreeSpec({
        ...
        "sourceParamNameCollisionWithEscapedParameter" {
            assertInvalidUsage(
                "sourceParamNameCollisionWithEscapedParameter",
                "collides",
                """
        ...
        // A CopyTarget* token has no target class to render here, so it is rejected up front
        // rather than leaking `{{cream:...}}` into the generated declaration's name.
        "unsupportedFunNameToken" {
            assertInvalidUsage(
                "unsupportedFunNameToken",
                "CopyTarget* naming token",
                """
                package callfrom.diag
