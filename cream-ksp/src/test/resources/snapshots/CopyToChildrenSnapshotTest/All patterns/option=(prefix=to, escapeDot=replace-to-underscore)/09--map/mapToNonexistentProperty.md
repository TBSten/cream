...
public sealed interface Source {
  @CopyToChildren.Map("missingTarget")
 ...
 * ```kt
 * val source = Source(...)
 * val target = source.to_Source_Child(targetName = targetName)
 * ```
 * 
 ...
 * ```kt
 * val source = Source(...)
 * val target = source.to_Source_Child(targetName = targetName, property = value)
 * ```
 * 
 * 
 * @see Source
