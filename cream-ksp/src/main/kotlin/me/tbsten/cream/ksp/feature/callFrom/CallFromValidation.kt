...
/**
 * Reject a `funName` embedding a `CopyTarget*` token: those render a target **class**, which
 * `@CallFrom` has none of, so the unexpanded `{{cream:…}}` would land in the generated
 * declaration's name. `resolveCallFromFunName` has already expanded the supported token, so any
 * residual one is unsupported.
 */
internal fun KSPLogger.validateNoUnsupportedFunNameToken(
    targetFunction: KSFunctionDeclaration,
    resolvedFunName: String,
): Boolean {
    if (!containsAnyCopyFunNameToken(resolvedFunName)) return true
    reportCreamError(
        InvalidCreamUsageException(
            message =
                "@$annotationName funName of ${targetFunction.displayName()} embeds a CopyTarget* naming token, " +
                    "which @$annotationName cannot render: it has no target class.",
            solution =
                "Use DefaultCopyFunctionName (the annotated function's own name) and/or plain text, " +
                    "e.g. funName = DefaultCopyFunctionName + \"FromArgs\".",
        ),
        targetFunction,
    )
    return false
