...
internal fun KSValueParameter.findMatchedProperty(
    ...
    if (generateSourceAnnotation is GenerateSourceAnnotation.CopyToChildren) {
        findSourcePropertyWithCopyToChildrenAnnotation(source, parameterName)
            ?.let { return it }
    }
    ...
    findSourcePropertyWithPropertyMappings(source, parameterName, copyMappingPropertyMappings)
        ?.let { return it }
    ...
    findSourcePropertyWithPropertyMappings(source, parameterName, combineMappingPropertyMappings)
        ?.let { return it }
 ...
 * @CopyToChildren.Map targets PROPERTY only, so no constructor-parameter fallback lookup is needed.
 */
private fun KSValueParameter.findSourcePropertyWithCopyToChildrenAnnotation(
    source: KSClassDeclaration,
    parameterName: String,
): KSPropertyDeclaration? =
    source
