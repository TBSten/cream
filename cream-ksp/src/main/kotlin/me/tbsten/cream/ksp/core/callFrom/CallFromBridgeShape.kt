...
/**
 * The source argument-holder parameter's name as an *identifier*, before escaping:
 * the source class's simple name in lowerCamelCase (e.g. `ProcessDataArgs` -> `processDataArgs`).
 *
 * This is the form to compare against `KSValueParameter.name` — KSP reports parameter names
 * unescaped, so a parameter declared `` `object` `` reads back as `object` and only matches a
 * source named `Object` here, not in [callFromSourceParamName]'s backticked rendering.
 */
internal fun callFromSourceParamIdentifier(source: KSClassDeclaration): String =
    source.simpleName
        .asString()
 ...
 * [callFromSourceParamIdentifier] escaped for keyword collisions. Shared with the feature layer,
 * which rejects the annotation when this name collides with one of the target function's own
 * parameters.
 */
internal fun callFromSourceParamName(source: KSClassDeclaration): String = callFromSourceParamIdentifier(source).escapeKotlinIdentifier()
 ...
 *
 * [findMappedSourceProperty] / [isExcluded] are the per-parameter generation rules (`.Map`
 * resolution and `@Exclude` handling), taken as ordinary parameters so a caller not driving
 * generation from an annotation can pass plain lambdas; cream's `@CallFrom` forwards
 * `CallFromSourceAnnotation`'s properties of the same names.
 */
internal fun callFromBridgeParameters(
    ...
    targetFunction.parameters.map { parameter ->
        ...
        CallFromBridgeParameter(
            ...
            isExcluded =
                matched != null &&
                    parameter.isExcludedFromCopy(matched, source, isExcluded),
        )
    }
...
internal fun callFromBridgeSignature(
    ...
    val keptParameterTypes =
        callFromBridgeParameters(source, targetFunction, findMappedSourceProperty, isExcluded)
            .filter { it.isKept }
