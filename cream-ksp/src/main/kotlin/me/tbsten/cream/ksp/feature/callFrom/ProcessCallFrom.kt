...
import me.tbsten.cream.ksp.core.common.CallFromSourceAnnotation
                        ...
                        groupUnits.forEach { unit ->
                            unit.sourceClasses.forEach { sourceClass ->
                                appendable.appendCallFromFunction(
                                    ...
                                    findMappedSourceProperty = unit.generateSourceAnnotation.findMappedSourceProperty,
                                    isExcluded = unit.generateSourceAnnotation.isExcluded,
...
private fun buildGenerationUnitImpl(annotated: KSAnnotated): CallFromGenerationUnit? {
    ...
    val generateSourceAnnotation =
        CallFromSourceAnnotation(
            annotation = callFromAnnotations.firstOrNull() ?: return null,
        )

    if (!logger.validateNoUnsupportedFunNameToken(
            targetFunction = targetFunction,
            resolvedFunName = resolveCallFromFunName(generateSourceAnnotation.funNameTemplate, targetFunction),
        )
    ) {
        return null
    }
