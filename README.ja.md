...
`@CopyMapping` および `@CombineMapping` では `@Exclude` を使えません
（コピー元/コピー先クラスがライブラリ側にあり、プロパティへのアノテーション付与が不可能です）。
代わりに `excludes` パラメータを使用してください — 各エントリは生成される (ターゲット側の) パラメータ名を指し、
そのパラメータの自動コピーのデフォルト値を外して必須にします。`properties = [Map(...)]` が `.Map` の
アノテーションレベル版であるのと同様に、これは `@Exclude` のアノテーションレベル版です。

```kt
@CopyMapping(
    source = LibXModel::class,
    target = LibYModel::class,
    excludes = ["shareProp"],  // shareProp の自動デフォルトを外す — 必須になる
)
private object Mapping

// 生成されるコード:
fun LibXModel.copyToLibYModel(
    shareProp: String,          // 必須 — `= this.shareProp` なし
    yProp: Int,
): LibYModel = ...
```

自動デフォルトの付くパラメータに一致しない `excludes` エントリは効果がなく、KSP の warning が出ます。
...
| **`cream.nonCopyableStrategy`**   | `@SealedCopy` の copy 不能な leaf (`object` / 互換 `copy` なし) の扱いのモジュール全体デフォルト。アノテーションの `nonCopyableStrategy` が `INHERIT` の場合に適用されます。 | `INHERIT`, `ERROR`, `RETURN_AS_IS`, `RETURN_NULL` | `ERROR` (実効)      |
...
### オプション 6. `cream.nonCopyableStrategy`

| デフォルト        | 設定可能な値                                                  |
|--------------|----------------------------------------------------------|
| `ERROR` (実効) | `INHERIT`, `ERROR`, `RETURN_AS_IS`, `RETURN_NULL` のいずれか。 |

`@SealedCopy` の [`nonCopyableStrategy`](#sealedcopy) — sealed 階層の copy 不能な leaf
(`object`、または互換な `copy(...)` / `@SealedCopy.Via` を持たない非 data class) の扱い —
のモジュール全体デフォルトを設定します。これはアノテーションごとの
`nonCopyableStrategy = NonCopyableStrategy.<...>` 引数のプロジェクトレベル版です。
`cream.defaultVisibility` と同様、アノテーション側が `INHERIT` のままの場合にだけ効き、
`@SealedCopy` にのみ影響します。

優先順位は以下の通りです:

1. アノテーションで明示した `nonCopyableStrategy`（`INHERIT` 以外）が常に優先されます。
2. 次に `cream.nonCopyableStrategy`（`ERROR` / `RETURN_AS_IS` / `RETURN_NULL` の場合）。
3. どちらも `INHERIT` の場合、実効 strategy はこのオプション追加前と同じく `ERROR` です。

例えば `cream.nonCopyableStrategy=RETURN_AS_IS` を設定すると、`object` leaf を含む階層への素の
`@SealedCopy` でも、各アノテーションに `nonCopyableStrategy = NonCopyableStrategy.RETURN_AS_IS` を
付けることなく、ビルドを失敗させる代わりに `is Empty -> this` が生成されます。
