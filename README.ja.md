...
プロジェクトレベル版です。設定可能な値は `CopyVisibility` enum に対応し、大文字小文字を区別しません
（`internal` と `INTERNAL` のどちらも受け付けます）。
...
`visibility = CopyVisibility.INTERNAL` を付けずに `internal` な copy 関数が生成されます。`INHERIT`
（デフォルト）のままにしておけば、これまで生成されていたコードは変わりません。
